...
function render() {
  ...
  const selectedExpenses = currencyExpenses.filter((expense) => getMonthKey(expense.date) === monthKey);
  const debtSum = currencyDebts.reduce((sum, debt) => sum + Number(debt.amount), 0);
  el.monthTotal.textContent = formatMoney(sumAmounts(thisMonthExpenses));
  el.todayTotal.textContent = formatMoney(sumAmounts(todayExpenses));
  el.recordsTotal.textContent = formatMoney(sumAmounts(selectedExpenses));
  ...
  el.debtTotal.textContent = formatMoney(debtSum);
  el.debtCount.textContent = `${currencyDebts.length} 筆`;
  el.settingsCurrency.textContent = currency;
  el.currencyButtons.forEach((button) => button.classList.toggle("active", button.dataset.currency === currency));
  renderLatestExpense(currencyExpenses[0]);
  renderExpenseList(el.recentList, currencyExpenses.slice(0, 4), "目前沒有紀錄");
